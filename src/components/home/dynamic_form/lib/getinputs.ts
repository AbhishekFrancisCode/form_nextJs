...
const generateValidations = (field: InputProps): YupBoolean | YupString | YupNumber | null => {
    ...
    for (const rule of field.validations) {
        switch (rule.type) {
            case 'isTrue': (schema as YupBoolean).isTrue(rule.message); break;
            case 'isEmail': (schema as YupString).email(rule.message); break;
            case 'minLength': (schema as YupString).min(rule.value as number, rule.message); break;
            case 'oneOf': (schema as YupString).oneOf([Yup.ref(rule.ref as string)], rule.message); break;
